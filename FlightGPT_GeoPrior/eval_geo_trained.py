...
from navgym.agents.SubgoalTracker import SubgoalTracker
from navgym.agents.VSV import VSV
...
API_CONFIG = {
    "api_key": "EMPTY",
    "api_base": "http://0.0.0.0:8000/v1",   #use your port
...
_cityrefer_data = _json.load(open('./data/cityrefer/objects.json'))

def _get_landmark_prior(ep, top_left, px_size):
    context = ""
    if not hasattr(ep, 'description_landmarks') or not ep.description_landmarks:
        return context
    map_name = ep.id[0] if isinstance(ep.id, tuple) else getattr(ep, 'map_name', None)
    if not map_name or map_name not in _cityrefer_data:
        return context
    block = _cityrefer_data[map_name]
    ...
    for name in ep.description_landmarks:
        ...
        if not roads:
            # fuzzy: match on meaningful words only
        ...
        if roads:
            obj = roads[0][1]
            px = [int((obj['position'][0]-top_left[0])/px_size), int((top_left[1]-obj['position'][1])/px_size)]
            context += f"[Geographic Prior] The instruction mentions '{name}'. This landmark is near map pixel {px}. Use this as spatial reference to locate the target described in the instruction.\n"
...
def run_nav_gym(citynavData, split, step, action_num):
    ...
    def process_sample(i):
        try:
            ...
            vsv = VSV(threshold=0.70)
            subgoal_metrics = {"total": 0, "completed": 0, "vsv_confirmed": 0}
            for step_idx in range(step):
                ...
                subgoal_context = tracker.get_context_for_prompt()  # tracked but not injected
                _prior = _get_landmark_prior(cur_citynavData.episode, navGym.top_left, navGym.px_real_size[0])
                result_str = agent.act(
                    ...
                    geographic_prior=_prior
                )
                tracker.update_from_model_output(result_str)
                if tracker.has_subgoals():
                    subgoal_metrics["total"] = len(tracker.subgoals)
                    active = tracker.get_active_subgoal()
                    if active and step_idx > 0:
                        try:
                            confirmed, sim, dist = vsv.verify(
                                drone_view_path=navGym.cur_rgb_drone,
                                subgoal_description=active["description"],
                                map_image_path=navGym.cur_whole_map,
                                landmark_bbox=active.get("landmark_bbox")
                            )
                            if confirmed:
                                pass  # tracker.advance() disabled - VSV cannot discriminate aerial landmarks
                                subgoal_metrics["completed"] += 1
                                subgoal_metrics["vsv_confirmed"] += 1
                        except Exception:
                            pass
...
def main():
    ...
    for split in ["easy", "medium", "hard"]:
        data_path = f"./data/citynav/citynav_test_unseen_{split}.json"
